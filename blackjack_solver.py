...
class Deck(np.ndarray):
    ...
    def get_hands_and_draws(self):
        ...
        draws = [[(hands.htoi[dest], card) for dest, card in draws[tuple(hand)]] for hand in hands]
        return hands, draws
...
class Dealer:
    def __init__(self, target, peeks, hit_soft_target):
        ...
        self.inds_stand_hands: np.ndarray
        self.prob_stand: np.ndarray
    ...
    def get_stand_probability(self):
        ...
        for h in self.inds_stand_hands:
            num_cards_left = DECK.total - HANDS.total - 1
            cards_to_draw = np.nonzero(HANDS[h])[0]
            denominator = np.where(
                np.all(deck_left >= HANDS[h], axis=-1),
                1
                / np.prod(
                    [num_cards_left - i for i in range(HANDS.total[h] - 1)],
                    axis=0,
                    dtype=float,
                ),
                0,
...
class Player:
    def __init__(self, options: PlayerOptions):
        self.options = options
        self.prob_if_stand: np.ndarray
...
def get_optimal_policy():
    ...
    policy = np.zeros((HANDS.len, DECK.len), dtype=np.uint8)
    exp_val = np.zeros(policy.shape)
    for h in range(HANDS.len - 1, -1, -1):
        hand = HANDS[h]
        exp_val_stand = PLAYER.prob_if_stand["win"][h] - PLAYER.prob_if_stand["lose"][h]
        if not DRAWS[h]:
            exp_val[h] = exp_val_stand
            continue
        for dealer_card in range(DECK.len):
            cards_left = DECK - hand
            if cards_left[dealer_card]:
                cards_left[dealer_card] -= 1
                num_cards = DECK.total - HANDS.total[h] - 1
                exp_val_hit = 0
                total_prob = 0
                for nxt, card_drawn in DRAWS[h]:
                    if cards_left[card_drawn]:
                        ...
                        total_prob += prob
                        exp_val_hit += exp_val[nxt][dealer_card] * prob
                exp_val_hit -= 1 - total_prob
                if exp_val_hit > exp_val_stand[dealer_card]:
                    policy[h, dealer_card] = 1
                    exp_val[h, dealer_card] = exp_val_hit
                else:
                    exp_val[h, dealer_card] = exp_val_stand[dealer_card]

                if HANDS.total[h] == 2:
                    # Surrender
                    if PLAYER.options.surrender and exp_val[h, dealer_card] < -0.5:
                        policy[h, dealer_card] = 2
                        exp_val[h, dealer_card] = -0.5
            else:
                exp_val[h, dealer_card] = np.nan
    return exp_val, policy
...
class Hands(np.ndarray):
    ...
    def __init__(self, _, htoi):
        self.htoi = htoi
        self.len = len(self)
        self.total: np.ndarray = np.sum(self, axis=-1, dtype=np.uint8)
...
def log():
    ...
    cond1 = np.logical_not(np.all(POLICY, axis=-1))
    cond2 = False
    for t in ops["totals"]:
        cond2 = np.logical_or(cond2, HANDS.total == t)
    ...
    hands = HANDS[inds]
    hands = [["".join([charmap[i] * c for i, c in enumerate(hand) if c])] for hand in hands]
    ...
    if ops["optimal_policy"]:
        pol = POLICY[inds].astype(int).tolist()
        print("\nOptimal Policy (PLAYER hand vs. dealer card)")
        print("2=Surrender  1=Hit  0=Stand")
        print(Tabulate(pol))

    game_exp_val = (EXP_VAL[0] @ DECK) / DECK.total
    ...
    if ops["plot"]:
        ...
        ax.imshow(POLICY[inds], cmap="binary")
        ...
        for i in range(DECK.len):
            for j in range(len(hands)):
                ax.text(
                    i,
                    j,
                    strmap[POLICY[inds][j, i]],
                    ha="center",
                    va="center",
                    color=clrmap[POLICY[inds][j, i]],
                )
...
if __name__ == "__main__":
    PLAYER_OPTIONS = PlayerOptions(**PLAYER_OPTS)
    ...
    EXP_VAL, POLICY = get_optimal_policy()
    log()
